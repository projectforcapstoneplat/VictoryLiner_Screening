...
function NotificationBell({ count }) {
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      {open && (
        <div style={{ position: 'absolute', top: 44, right: 0, width: 260, background: 'var(--surface-card)', borderRadius: 12, boxShadow: '0 12px 28px rgba(0,0,0,0.22)', padding: 16, zIndex: 60 }}>
          <strong style={{ fontSize: 'var(--text-sm)' }}>Awaiting your review</strong>
          <p style={{ fontSize: 'var(--text-xs)', opacity: 0.7, margin: '6px 0 0' }}>
            {count > 0
              ? `${count} candidate${count === 1 ? '' : 's'} fully screened and ready for a decision.`
...
function UserMenu({ profile, nav }) {
  const [open, setOpen] = useState(false);
  const handleSignOut = async () => { await signOut(); nav('home'); };
  return (
    <div style={{ position: 'relative' }}>
      <button onClick={() => setOpen((o) => !o)} style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
        ...
        <span style={{ color: 'rgba(255,255,255,0.8)', display: 'flex' }}>{ICONS.chevron}</span>
      </button>
      {open && (
...
export function HrShell({ active, nav, profile, notificationCount = 0, children }) {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--surface-page-alt)', fontFamily: 'var(--font-ui)' }}>
      <aside style={{ width: 232, flexShrink: 0, background: 'var(--surface-card)', borderRight: '1px solid var(--border-hairline)', display: 'flex', flexDirection: 'column' }}>
        <nav style={{ flex: 1, padding: '20px 14px', display: 'flex', flexDirection: 'column', gap: 4 }}>
          {items.map((item) => (
            <SidebarLink key={item.key} item={item} isActive={active === item.key} onClick={() => nav(item.key)} />
          ))}
        </nav>
      ...
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
        <header style={{
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 12, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 7, fontFamily: 'var(--font-display)', flexShrink: 0 }}>
              ...
              <span style={{ color: 'rgba(255,255,255,0.7)', fontWeight: 600, fontSize: 10, letterSpacing: 2, textTransform: 'uppercase' }}>Careers</span>
            </div>
            <span style={{ color: 'rgba(255,255,255,0.5)' }}>|</span>
            <span style={{ color: '#fff', fontWeight: 500, fontSize: 'var(--text-sm)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
              HR Command Center
            </span>
          </div>
        ...
        <main style={{ flex: 1, padding: '28px 32px 60px', minWidth: 0 }}>{children}</main>
      </div>
    </div>
  );
}
